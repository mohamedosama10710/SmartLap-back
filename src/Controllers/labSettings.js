...
export const updateSchedule = async (req, res, next) => {
  try {
    ...
    let saveSchedule = await lapsettingsModel.findOneAndUpdate(
      {},
      { workingHours, slotDuration, offDays },
...
export const getSchedule = async (req, res, next) => {
  try {
    ...
    if (labSettings.offDays.includes(dayName)) {
      return res.status(400).json({
    ...
    const [startHour, startMin] = labSettings.workingHours.start
      .split(":")
      .map(Number);
    const [endHour, endMin] = labSettings.workingHours.end
      .split(":")
      .map(Number);
